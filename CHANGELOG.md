...
- `agent_skill_guard.py`: loosened the "fake-compliance data exfiltration" and "response appending directive" SCH patterns — they previously required a specific word order/literal word and missed reordered or reworded variants ("X mandates: route ...", "include in all outputs: ..."). The broadened "fake-compliance" pattern also now requires an actual destination (`to`/`at <address>`) after the action verb — an earlier draft matched on the qualifier/verb alone and false-positived on benign compliance/audit prose, caught in review before merge. Same regex source and fix as npm v4.32.3. (nkratk/llm-trust-guard-python#2, companion to nkratk/llm-trust-guard#2)
