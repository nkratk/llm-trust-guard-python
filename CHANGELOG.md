...
- **Permanent ReDoS-safety regression test** (`tests/test_redos_safety.py`): extracts every `re.<func>(pattern, ...)` call in `src/` — compile/search/match/fullmatch/sub/subn/split/findall/finditer — and stress-tests each against a fixed adversarial seed corpus using a scaling-ratio check (not a single absolute-time threshold — CPython's regex engine is slow enough that some already-fixed, genuinely-linear patterns overlap in absolute time with real quadratic bugs at any single seed size; growth ratio across two sizes cleanly separates them regardless of the constant factor). Writing this test found two real bugs the v0.21.4 manual sweep had missed. The extractor originally matched only `re.compile(...)` calls; broadened after the npm sibling's analogous `pattern:`/ALL-CAPS-`const` extractor turned out to miss ~28% of its regex literals (and did hide one real bug there) — this repo's broader extraction found no new bugs among the 81 additional `re.search`/`re.sub`/etc. call sites, but that's worth having confirmed rather than assumed.
